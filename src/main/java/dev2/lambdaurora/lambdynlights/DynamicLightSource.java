...
public interface DynamicLightSource {
	...
	default boolean isDynamicLightEnabled() {
		return LambDynLightsMod.config.isDynamicLightsEnabled() && LambDynLightsMod.get().containsLightSource(this);
	}
	...
	default void setDynamicLightEnabled(boolean enabled) {
		...
		if (enabled)
			LambDynLightsMod.get().addLightSource(this);
		else
			LambDynLightsMod.get().removeLightSource(this);
	}
